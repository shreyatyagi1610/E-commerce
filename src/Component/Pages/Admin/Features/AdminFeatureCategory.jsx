...
import { getFeatures,deleteFeatures } from '../../../../ActionCreators/FeatureCategoryActionCategory'

export default function AdminFeaturesCategory() {
    ...
    <div className="container-fluid my-3">
      <div className='row'>
        ...
        <div className='col-md-9 mb-3'>
          <h5 className='bg-primary text-light  text-center p-2'>Features<Link to="/Admin/Features/Create"><i className='fa fa-plus text-light float-end'></i></Link></h5>
            ...
            <tbody>
              {
                FeaturesCategoryStateData.map(item => {
                  return <tr key={item.id}>
                    <td>{item.id}</td>
                    <td>{item.name}</td>
                    <td><span dangerouslySetInnerHTML={{__html:item.icon}}/></td>
                    ...
                    <td><Link to={`/admin/Features/update/${item.id}`} className='btn btn-primary'><i className='fa fa-edit'></i></Link></td>
